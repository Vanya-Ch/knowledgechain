document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (!user || !Array.isArray(user.likedTopics) || user.likedTopics.length === 0) {
            container.innerHTML += '<p>Ви ще не лайкали жодного топіка.</p>';
            return;
        }

        // Отримуємо лайкнуті топіки
        const res = await fetch('/api/topics/liked', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
            body: JSON.stringify({ ids: user.likedTopics })
        });

        if (!res.ok) throw new Error('Failed to load liked topics');
        ...
        const renderTopics = () => {
            ...
            pageTopics.forEach(topic => {
                ...
                const isLiked = topic.likes.includes(user._id);
        ...
        container.innerHTML += '<p>Помилка при завантаженні даних.</p>';
    }
});
